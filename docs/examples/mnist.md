...
position forms a 136-feature patch (272 literals). Adaptive thresholding turns the flat
background on and the strokes off (as in the reference implementations), which starts slower
than a plain `pixel > 0.3` threshold (≈ 74 % vs 96 % after one epoch with 200 clauses/class)
but is the recipe used for the best published results.
