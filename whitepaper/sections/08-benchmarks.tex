...
\thead{Software} & PyTorch 2.10.0+cu128 / CUDA 12.8, Python 3.10.11, \ttlibname 0.1.3\\
\thead{Measured} & 17 September 2026\\
...
Figure~\ref{fig:bench-batch} is what it buys: on the GPU, training throughput rises from 434
examples/s at batch 1 to 219{,}330 at batch 4096 --- a factor of 505 for feeding the same
kernel more work.

\begin{figure}[htbp]
\centering
\input{figures/fig-bench-batch}
...
At batch 256 the same model trains at 88{,}771 examples/s on the GPU against 404 on the CPU
--- $220\times$. Inference behaves differently: the CPU forward pass is already cheap, so the
gap there tops out around $30\times$, reaching $1.05\times10^{6}$ examples/s on the GPU.
...
model size, from $50\times$ at 50 clauses per class to $275\times$ at 4000, and from
$70\times$ at 128 features to $282\times$ at 8192.

\begin{figure}[htbp]
\centering
\input{figures/fig-bench-scale}
...
trains at 25{,}864 examples/s against 95 on the CPU.
...
(Table~\ref{tab:zoo}). The convolutional models are the outlier at roughly $3\times$ the cost
per example on the GPU, which is arithmetic rather than overhead: a $10\times10$ window over a
$28\times28$ image is 361 patches, and every patch is evaluated against every clause. Their
\emph{inference} cost is the striking one --- 24{,}805 examples/s against 529{,}243 for the
flat machine --- because prediction is where the patch dimension has no feedback work to hide
behind.

\begin{table}[htbp]
\centering
\small\sffamily\sloppy
\caption{All models on a $28\times28$ Boolean input, batch 128. Throughput in examples/s.
  Each model is measured at its natural configuration: 5000 clauses over 784 flat features,
  against 2000 clauses over 136 per-patch features for the convolutional ones. The CPU column
  therefore compares two different amounts of work, not two implementations of the same one.}
\label{tab:zoo}
...
\ttmidrule
\clsname{TsetlinMachine}            & 269 & 49{,}794 & 185$\times$ & 37{,}801 & 529{,}243 & 14$\times$\\
\clsname{TsetlinMachine} (weighted) & 267 & 49{,}534 & 185$\times$ & 33{,}830 & 482{,}612 & 14$\times$\\
\clsname{CoalescedTsetlinMachine}   & 177 & 43{,}563 & 246$\times$ & 34{,}536 & 527{,}674 & 15$\times$\\
\clsname{RegressionTsetlinMachine}  & 213 & 56{,}950 & 267$\times$ & 39{,}960 & 562{,}748 & 14$\times$\\
\clsname{ConvTsetlinMachine}        & 535 & 15{,}275 &  29$\times$ &      676 &  24{,}805 & 37$\times$\\
\clsname{ConvCoalescedTsetlinMachine} & 389 & 15{,}356 & 39$\times$ & 658 & 24{,}565 & 37$\times$\\
\ttbotrule
\end{tabularx}
\end{table}

GPU peak memory during training is 262\,MB for the flat models and 182--225\,MB for the
convolutional ones at these configurations --- small enough that clause budget, not memory, is
the binding constraint on most cards.
...
$(C,2F)$ state --- is 92\% of the 625\,ms CPU call and 42\% of the 2.8\,ms GPU call. A
standalone \code{torch.binomial} draw of the same shape costs 266.7\,ms on the CPU against
0.22\,ms on the GPU, making it the single most expensive operation in the commit.

\begin{figure}[htbp]
\centering
\input{figures/fig-bench-breakdown}
\caption{Stage timings for one update (5000 clauses, 784 features, batch 256). Note the log
  scale: the CPU total is 625.3\,ms and the GPU total 2.79\,ms. Everything before the commit
...
More CPU threads buy surprisingly little in training --- $1.3\times$ from 1 to 32 threads
...
Inference scales much better, $8.6\times$ over the same range, because the forward pass is
...
\begin{figure}[htbp]
\centering
\input{figures/fig-bench-threads}
...
\centering
\small\sffamily\sloppy
\caption{End-to-end MNIST runs with \code{Trainer.fit} included. The flat model's accuracy
  matches across devices to within 0.0006, as it should --- only the wall clock moves.
  The single-epoch convolutional run is noisier (0.847 against 0.804): that is the variance of
  one pass over 10\,000 examples, not a device difference.}
\label{tab:e2e}
...
\multirow{2}{*}{\clsname{TsetlinMachine} 500/class}
  & CPU  & 60{,}000 & 3 & 114.05 & 0.9542\\
  & GPU  & 60{,}000 & 3 & \textbf{1.53} & 0.9548\\
\ttmidrule
\multirow{2}{*}{\clsname{ConvTsetlinMachine} 200/class, $10\times10$}
  & CPU  & 10{,}000 & 1 & 16.10 & 0.8470\\
  & GPU  & 10{,}000 & 1 & \textbf{0.70} & 0.8035\\
\ttbotrule
\end{tabularx}
\end{table}

A 114-second epoch becomes a 1.5-second one ($75\times$); the convolutional run goes from 16
seconds to 0.7 ($23\times$). That is the difference between a hyper-parameter sweep you plan
...
\begin{figure}[htbp]
\centering
\input{figures/fig-bench-fidelity}
